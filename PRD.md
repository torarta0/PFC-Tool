...
- **前额叶助手 (Assistant)**: 接入 Gemini API 的 "Neuroscience Coach"，提供任务拆解、冲动缓冲、镜面反馈及语义化记录功能。
...
- **兑换流程**: 
  ...
  3. **冲动缓冲 (Impulse Buffer)**: 若奖励属于“感官愉悦”或“精神放松”类别，强制进入 1 分钟冷静期（含深呼吸引导与理性问答）。
  4. 成功后，弹出成功提示（含撤回按钮），增加一条 `RedemptionLog`，更新 `spentPoints`。
- **背包系统 (Inventory)**: 
  - **入口**: 奖励商店“我的仓库”选项卡。
...
- **镜面反馈**: 助手会根据用户近期行为风格生成一段个性化的“指挥官评价”。

### 3.6 前额叶助手 (Assistant)
- **角色设定**: "Neuroscience Coach"，回复简短、理性、鼓励、去情绪化。
- **核心功能**:
  - **任务拆解 (Task Architect)**: 将复杂任务拆解为 15 分钟微任务。
  - **优先级建议**: 根据当前积分（PFC 能量）建议任务优先级。
  - **冲动缓冲 (Impulse Buffer)**: 作为负面行为或即时奖励的缓冲带。
  - **镜面反馈 (Mirror Feedback)**: 提供元认知洞察，关联行为因果（如睡眠与专注度的关系）。
  - **语义化记录 (Semantic Logger)**: 识别模糊输入并自动转化为结构化日志记录。
...
- **外观设置 (Appearance)**:
  ...
  - **字体大小**: 提供 `Small`, `Medium`, `Large` 三档调节。已针对全局可读性进行深度优化，确保所有微小文本（如时间戳、标签、描述）在各档位下均保持清晰易读，最小字号提升至 `text-xs` (12px) 或以上。
- **等级系统**:
...
- **顶部导航 (TopAppBar)**: 
  - **左侧**: 统一风格的 `MaterialIcon` (military_tech) 图标与应用名称，取代原始占位图。
  - **中间 (仅桌面端)**: 选项卡导航。
  - **右侧**: 实时显示等级与累积积分。
...
- **Hero 区域 (状态看板)**: 
  ...
  - **注意**: 仅在“指挥中心”和“设置”页面显示。在“日志”、“商店”和“数据分析”页面隐藏此区域，以提供更专注的功能环境。
...
- **图标系统**: 全面采用 `Material Symbols` 矢量图标，确保视觉一致性，杜绝位图占位符。
- **配色主题**: 
  - `Bright`: 核心明亮主题。
  - `Dark`: 深邃暗黑主题。
  - `Nordic Moss`: 自然深绿主题。
...
- **可读性优化**: 全局强化文本对比度与字号平衡，特别针对移动端小屏幕进行了精细化适配。
...
5. **资产**: 全面使用 `Material Symbols` 作为核心图标资产，仅在用户手动上传或特定奖励展示时使用图片。
6. **可读性**: 严格遵循全局字号平衡原则，确保所有功能性文本在移动端清晰可见。
